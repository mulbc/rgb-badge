...
def read_sources(project_dir):
    ...
    if (len(sheets) != 10 or children(root, 'symbol') or
            ...
            sum(properties(s)['Sheetfile'] == 'usb-interface.kicad_sch' for s in sheets) != 1):
        raise ValueError('Expected four matrix sheets plus driver, rows, controller, two USB logic sheets and USB interface, with no root components')
    ...
    for sheet in sheets:
        ...
        if filename in {'driver.kicad_sch', 'rows.kicad_sch', 'controller.kicad_sch', 'usb-conditioning.kicad_sch', 'usb-permission.kicad_sch', 'usb-interface.kicad_sch'}:
