...
def read_sources(project_dir):
    ...
    if (len(sheets) != 9 or children(root, 'symbol') or
            ...
            sum(properties(s)['Sheetfile'] == 'usb-permission.kicad_sch' for s in sheets) != 1):
        raise ValueError('Expected four matrix sheets plus driver, rows, controller and two USB logic sheets, with no root components')
    ...
    for sheet in sheets:
        ...
        if filename in {'driver.kicad_sch', 'rows.kicad_sch', 'controller.kicad_sch', 'usb-conditioning.kicad_sch', 'usb-permission.kicad_sch'}:
