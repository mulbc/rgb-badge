...
`python3 tools/check-power-libraries.py` checks the phase-one exact `BQ25616JRTWT` and `TPS631000DRLR` symbols against their manufacturer pin tables and verifies every RTW/DRL pad, the charger exposed-pad copper/paste split, layers and pin-1 marks. The [power-library audit](../hardware/coupon/rev-a/power-library-audit.md) records source hashes and the intentionally unresolved connector, X2QFN, DSJ and fuel-gauge packages. `check-kicad.sh` requires nonempty native symbol plus fabrication/copper/paste exports for both new footprints; a native render review is still required before capture uses them.
