...
`python3 tools/check-power-libraries.py` checks the controlled `BQ25616JRTWT`, `TPS631000DRLR`, `TLV75533PDBVR`, `SN74LVC1G04DBVR`, `INA232AIDDFR` and base-suffix `TPD4E05U06DQAR` symbols against manufacturer pin tables and verifies every RTW/DRL/DBV/DDF/DQA land, the charger exposed-pad copper/paste split, layers and pin-1 marks. The [power-library audit](../hardware/coupon/rev-a/power-library-audit.md) records source hashes and intentionally unresolved connector, X2QFN, DSJ and fuel-gauge packages. `check-kicad.sh` requires nonempty native symbol plus fabrication/copper/paste exports for every new footprint; a native render review is still required before capture uses them.
