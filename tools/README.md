...
The historical power arithmetic check now prints the unresolved USB/input blockers explicitly. Run `python3 tools/check-power-design.py --require-usb-closure` to require a completed USB topology: it currently exits 1 by design under ADR 0009. Library/export success is not USB-current closure. The validator separately rejects out-of-range ILIM settings, insufficient source allowances and use of ILIM bounds for BC1.2-classified sources.
