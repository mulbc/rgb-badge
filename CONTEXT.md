...
- Coupon schematic: the matrix and TLC59581 driver passed owner KiCad 10.0.6 ERC/exported-netlist and six-page visual review at `8e95eb0`. Exact row-selection libraries passed native rendering review at `0c71860`; the complete row capture then passed zero-violation ERC, 335-item / 1,290-pin native connectivity and seven-page visual review at `eb4129b`. Controller and power source remain uncaptured, and 3.3 V N-MOS behavior plus hardware VLED inhibition remain explicit review/measurement items. See the [matrix](hardware/coupon/rev-a/matrix-capture.md), [driver](hardware/coupon/rev-a/driver-capture.md), [row-library](hardware/coupon/rev-a/row-library-audit.md), [row-capture](hardware/coupon/rev-a/row-capture.md) and [native row review](docs/development/row-capture-review-eb4129b.md) records.
- Repository workflow: owner granted standing permission on 2026-09-07 to merge PRs after applicable checks pass. LED library PR #2, matrix PR #3, driver PR #4 and row-library PR #5 are merged. Row-capture PR #6 has passed its native merge gate; controller capture is next.
