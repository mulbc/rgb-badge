...
- Active circuit checkpoint (2026-09-16): [two staged USB permission sheets](hardware/coupon/rev-a/permission-capture.md) now contain five dual Schmitt buffers, fourteen logic gates, default resistors, decoupling and test pads (61 new items / 176 pins). Source totals: ten pages, 417 items / 1,536 pins. The checker traces the actual wires and verifies all 1,024 stable input combinations. Native ERC/XML/render review is pending. Detectors, supervisors, application inputs and charger/ILIM outputs remain explicit test boundaries; physical startup/boost circuitry and the complete power section remain unfinished. This supersedes the library-only "no owner action" checkpoint below: the next owner action is a combined native export of the captured logic and new libraries.
- Permission-library increment (2026-09-16): five exact LVC gates and TPS3808G01DBVR now have checked local symbols (32 pins), with a new DBV6 land pattern. The existing DBV5 nominal body drawing and courtyard were corrected without changing its copper. See the [permission-library audit](hardware/coupon/rev-a/permission-library-audit.md). Native rendering is pending and will be batched with circuit capture; no owner action is required for this library-only checkpoint. Supervisor thresholds/startup inhibition and the ILIM switch remain to be qualified before connection.
