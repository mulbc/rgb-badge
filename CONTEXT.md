...
- Power-library review update: the native run at `c84f8ce` passed its staged electrical checks, but drawing/render review found a charger ground-pad short, wrong converter stencil geometry and overlapping symbol headings. The corrected native exports at `3638b1d` passed first-author review. The follow-on row layout and stale-note repairs pass 75 host tests and await native verification. PR #8 remains draft. The [finding record](docs/development/power-library-review-c84f8ce.md) also tracks the existing row-page title-block collision and stale annotations, now repaired in source but awaiting native confirmation before the power/input merge. See the [correction review](docs/development/power-library-review-3638b1d.md).
