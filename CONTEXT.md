...
- Coupon schematic: the merged matrix and TLC59581 driver passed owner KiCad 10.0.6 ERC/exported-netlist and six-page visual review at `8e95eb0`. Exact row-selection libraries passed native rendering review at `0c71860` and PR #5 is merged. The row-capture branch adds U2, C2, sixteen P-MOSFETs, sixteen N-MOSFETs and 37 resistors; source checks require 335 PCB items / 1,290 physical pins in the complete native XML. Its KiCad ERC/XML/seven-page PDF review is pending. Controller and power source remain uncaptured, and 3.3 V N-MOS behavior plus hardware VLED inhibition remain explicit review/measurement items. See the [matrix](hardware/coupon/rev-a/matrix-capture.md), [driver](hardware/coupon/rev-a/driver-capture.md), [row-library](hardware/coupon/rev-a/row-library-audit.md) and [row-capture](hardware/coupon/rev-a/row-capture.md) records.
- Repository workflow: owner granted standing permission on 2026-09-07 to merge PRs after applicable checks pass. LED library PR #2, matrix PR #3, driver PR #4 and row-library PR #5 are merged. Row-capture native review is the next merge gate.
