...
- Coupon schematic: the merged matrix and TLC59581 driver contain 264 PCB items and 1,094 physical pins; owner KiCad 10.0.6 ERC, exported-netlist and six-page visual review passed at `8e95eb0`. Exact row-selection symbols and footprints for `74HC4514PW,118`, `DMP2066LSN-7`, `2N7002K-7` and `ERJ-2RKF1001X` passed owner KiCad 10.0.6 export/ERC and full-resolution visual review at `0c71860`, with 3.3 V N-MOS behavior and VLED hardware inhibition explicitly left for review/measurement. Row stages, controller and power source remain uncaptured. See the [matrix](hardware/coupon/rev-a/matrix-capture.md), [driver](hardware/coupon/rev-a/driver-capture.md), [row-library](hardware/coupon/rev-a/row-library-audit.md) and [native row-library review](docs/development/row-library-review-0c71860.md) records.
- Repository workflow: owner granted standing permission on 2026-09-07 to merge PRs after applicable checks pass. LED library PR #2, matrix PR #3 and driver PR #4 are merged. The row-library branch is eligible to merge after its passed native review; row-stage capture is next.
