...
The driver draft now selects `ERJ-2RKF3922X` (39.2 kΩ) for IREF and `GRM155R71C104KA88D` (100 nF) for local decoupling, plus four `ERJ-2RKF1003X` 100 kΩ input pull-downs; see its audit and current-calculation limits. The VBUS protection device, fuse strategy, regulator inductors/capacitors, current shunt, row-gate resistors and remaining thermal/layout values remain open. They must not be inferred from this short list.
