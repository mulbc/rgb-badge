...
| PWR-005 | With the switch OFF, charging, cell qualification and charge status shall operate autonomously from a positively identified BC1.2 charging source or a USB-C source advertising 1.5 A/3 A; an SDP, default-only or unclassified source shall remain in charger standby. | State-matrix test | Coupon/final |
...
| USB-004 | On an SDP, charging shall start only after native USB configuration while the switch is ON and shall return to charger standby on suspend, detach or reset. The request shall default inactive and shall not grant the resistor-programmed high-current mode. | USB protocol/current trace plus reset and fault-injection tests (ADR 0010) | Coupon/final |
| USB-005 | Configured-SDP operation shall reserve current within the port's 500 mA allowance for all VBUS auxiliaries and programming-network errors. Use the lower external ILIM setting; only source-detection hardware may enable its parallel boost branch. | Worst-case complete-port budget including qualified total resistor error, GPIO permission tests and measured transitions (ADRs 0011/0012) | Coupon/final |
